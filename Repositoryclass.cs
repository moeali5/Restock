...
{
    public partial class Repositoryclass : Form
    {
        ...
        private void Repositoryclass_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (txtFilePath.Text.Contains("PJR"))
                {
                    PJR pjr = new PJR();
                    foreach (XElement level0Element in XElement.Load(txtFilePath.Text).Elements("TransmissionHeader"))
                    {
                        pjr.LocationID = Convert.ToInt32(level0Element.Element("StoreLocationID").Value);
                    }
                    foreach (XElement level0Element in XElement.Load(txtFilePath.Text).Elements("JournalReport"))
                    {
                        foreach (XElement level1Element in level0Element.Elements("JournalHeader"))
                        {
                           
                            pjr.BeginDate = Convert.ToDateTime(level1Element.Element("BeginDate").Value + " " + level1Element.Element("BeginTime").Value);
                            pjr.EndDate = Convert.ToDateTime(level1Element.Element("EndDate").Value + " " + level1Element.Element("EndTime").Value);
                        }
                        foreach (XElement level2Element in level0Element.Elements("SaleEvent"))
                        {
                            pjr.CreateDate = Convert.ToDateTime(level2Element.Element("BusinessDate").Value);
                            pjr.Transaction_ID = Convert.ToInt32(level2Element.Element("TransactionID").Value);
                            pjr.Cashier_ID = Convert.ToInt32(level2Element.Element("CashierID").Value);
                            pjr.Register_ID = Convert.ToInt32(level2Element.Element("RegisterID").Value);
                            foreach (XElement level3Element in level2Element.Elements("TransactionDetailGroup"))
                            {
                                foreach (XElement level4Element in level3Element.Elements("TransactionLine"))
                                {
                                    pjr.Transaction_Status = (level3Element.Element("TransactionLine")).Attribute("status").Value;
                                    foreach (XElement level5Element in level4Element.Elements("ItemLine"))
                                    {
                                        pjr.Department_ID = Convert.ToInt32(level5Element.Element("MerchandiseCode").Value);
                                        foreach (XElement level6Element in level5Element.Elements("ItemCode"))
                                        {
                                            pjr.UPC_Code = (level6Element.Element("POSCode").Value);
                                            pjr.Modifier = Convert.ToInt32(level6Element.Element("POSCodeModifier").Value);
                                        }
                                        pjr.Transaction_Entry_Method = (level5Element.Element("EntryMethod")).Attribute("method").Value;
                                        pjr.Description = (level5Element.Element("Description").Value);
                                        pjr.Selling_Unit = Convert.ToInt32(level5Element.Element("SellingUnits").Value);
                                        pjr.Unit_Price = Convert.ToDouble(level5Element.Element("ActualSalesPrice").Value);
                                        pjr.Unit_Sold = Convert.ToInt32(level5Element.Element("SalesQuantity").Value);
                                        pjr.Total_Price = Convert.ToDouble(level5Element.Element("SalesAmount").Value);
                                        pjr.Transaction_Type = "Item";
                                    }
                                    foreach (XElement level5Element in level4Element.Elements("FuelLine"))
                                    {
                                        pjr.Fuel_Line = Convert.ToInt32(level5Element.Element("FuelPositionID").Value);
                                        pjr.Fuel_Grade_ID = Convert.ToInt32(level5Element.Element("FuelGradeID").Value);
                                        pjr.Transaction_Type = "Fuel";                                     
                                        pjr.Fuel_Description = (level5Element.Element("Description").Value);
                                        pjr.Actual_Sale_Price = Convert.ToDouble(level5Element.Element("ActualSalesPrice").Value);
                                        pjr.Sale_Amount = Convert.ToDouble(level5Element.Element("SalesAmount").Value);
                                        pjr.Sale_Qty = Convert.ToDouble(level5Element.Element("SalesQuantity").Value);
                                        pjr.Transaction_Entry_Method = (level5Element.Element("EntryMethod")).Attribute("method").Value;
                                        pjr.Department_ID = Convert.ToInt32(level5Element.Element("MerchandiseCode").Value);

                                    }
                                            string s = "insert into Location_Transation_Log values(" + pjr.LocationID + "," + pjr.Transaction_ID + ",'" + pjr.BeginDate +
                                     "','" + pjr.EndDate + "'," + pjr.Register_ID + ",'" + pjr.Cashier_ID + "'," + pjr.Department_ID + ",'" + pjr.UPC_Code + "'," + pjr.Fuel_Line +
                                     "," + pjr.Fuel_Grade_ID + ",'" + pjr.Transaction_Type + "','" + pjr.Transaction_Status + "','" + pjr.Fuel_Description +
                                     "'," + pjr.Actual_Sale_Price + "," + pjr.Sale_Qty + "," + pjr.Sale_Amount + "," + pjr.Transaction_Entry_Method + "," + pjr.Modifier +
                                     ",'" + pjr.Description + "'," + pjr.Selling_Unit + "," + pjr.Unit_Price + "," + pjr.Unit_Sold + "," + pjr.Total_Price + ",0,0,0,0,0,0,'" + pjr.CreateDate.ToString() + "','" + DateTime.Now.ToString("yyyy/mm/dd") + "'," + 0 + ")";
